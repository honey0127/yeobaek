...
- [x] **플래너 편집**: 순서 드래그(ItemTouchHelper)·스와이프 삭제(되돌리기)·"이 순서로 다시 계산"
  (`keep_order=true` 재스케줄)·코스 저장/불러오기(`CourseStore`, SharedPreferences+Gson).
  보관함은 **내 여백**(`MyPageActivity`)에서 열고, 열 때 서버로 다시 계산한다(혼잡도는 시간이
  지나면 달라지므로 저장 당시 도착시각을 그대로 보여주면 틀린 정보가 된다).
- [x] **히트맵 시각화 개선**: 원형 농도 오버레이(`HeatOverlay`, 레벨별 알파·반경) + 범례 카드
  + `한적한 곳만` 필터(지도·추천 목록 동시 적용). 레벨을 모르는 곳은 원을 그리지 않는다
  (예보권 밖을 '보통'으로 칠하면 없는 정보를 지어내는 셈).
