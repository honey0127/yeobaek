...
- **코스 편집**: 플래너에서 **길게 눌러 드래그**하면 순서를 바꾸고 **옆으로 밀면** 지운다(되돌리기 제공).
  편집한 순서는 `이 순서로 다시 계산`(`keep_order=true`)으로 도착시각·혼잡도를 갱신한다.
- **내 여백(`MyPageActivity`)**: 저장한 코스 보관함 + 여백 스탬프 + 에코 포인트·배지 진행바를
  한 화면에. 저장한 코스를 탭하면 **그 자리에서 다시 계산해서** 연다(혼잡도는 시간이 지나면
  달라지므로 저장 당시 도착시각을 그대로 보여주지 않는다). 홈의 🌱·旅 칩에서 진입.
- **혼잡 히트맵**: 명소마다 반투명 원(`HeatOverlay`)을 깔아 붐비는 구역이 드러나게 한다.
  레벨이 높을수록 진하고 넓다. 범례 카드와 `한적한 곳만` 필터(지도·추천 목록 동시 적용) 제공.
  레벨을 모르는 곳(예보권 밖)은 원을 그리지 않고 회색 라벨로만 둔다.
- **화면이 꺼져도 급증 알림**: 플래너의 `🔔 화면 꺼도 알림 받기` → 포그라운드 서비스
  (`SurgeMonitorService`)가 SSE 를 계속 구독해 시스템 알림을 띄운다(회복하면 알림도 사라짐).
  상시 알림의 `감시 끄기` 로 종료. 출시 시 Play Console 에 포그라운드 서비스 용도 선언 필요.
